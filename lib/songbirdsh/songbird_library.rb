...
class Songbirdsh::SongbirdLibrary
  ...
  def count
    with_db {|db| db[:media_items].count }
  end
  
  def row id
    with_db {|db| db[:media_items][:media_item_id=>id] }
  end

  def path row
    CGI.unescape(row[:content_url].slice(7..-1)) if row[:content_url] =~ /^file/
  end
